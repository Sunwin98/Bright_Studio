...
## อัปเดต 2026-07-15 — ประวัติไฟล์, Asset Preview และ Script Builder

### ประวัติไฟล์และกู้คืน

- เพิ่ม `app/core/history.py`, `app/api/history.py` และหน้า `web/js/pages/history.js`
- เก็บ snapshot ไว้ที่ `.storage/history` พร้อม metadata และจำกัดรายการล่าสุดไม่เกิน 100 รายการ
- รองรับดูรายละเอียด, สร้าง snapshot เอง, restore และลบประวัติ
- ก่อน restore จะ snapshot สถานะปัจจุบันอัตโนมัติ เพื่อย้อนกลับได้อย่างปลอดภัย
- เชื่อมกับการแก้ปัญหาอัตโนมัติของ Add-on Checker, การบันทึก Script Lab และการแก้ฟิสิกส์
- API หลัก: `GET /api/history`, `POST /api/history/snapshot`, `GET /api/history/{snapshot_id}`, `POST /api/history/{snapshot_id}/restore`, `DELETE /api/history/{snapshot_id}`

### Asset Preview

- เพิ่ม `app/core/assets.py`, `app/api/assets.py` และหน้า `web/js/pages/assets.js`
- สแกนได้ทั้งโฟลเดอร์และไฟล์ archive พร้อมจัดกลุ่ม texture, model, entity, attachable, animation, JSON, script และไฟล์ทั่วไป
- แสดงภาพ texture จริง, geometry id, จำนวน bone และ reverse reference ของไฟล์ JSON ที่อ้างถึงกัน
- API หลัก: `POST /api/assets/scan` และ `GET /api/assets/file?path=...`
- เป็นโหมดอ่านอย่างเดียว จึงไม่แก้ไขไฟล์ต้นฉบับ
- ข้อจำกัดปัจจุบัน: model แสดง metadata ก่อน ยังไม่มีตัว viewer 3D แบบหมุนได้ และไฟล์ TGA ยังไม่ preview ใน browser

### Script Builder

- เพิ่ม `app/core/scriptbuilder.py`, `app/api/scriptbuilder.py` และหน้า `web/js/pages/scriptbuilder.js`
- ออกแบบเป็น 3 ขั้นตอน: เลือก Behavior Pack → ต่อบล็อก → สร้างไฟล์
- รองรับ event: ใช้ไอเทม, เข้าโลก, ตี Entity, ทำลายบล็อก และ tick
- รองรับบล็อกเงื่อนไข/การทำงาน เช่น ตรวจไอเทม, ตั้งช่วงเวลา, ส่งข้อความ, เล่นเสียง, effect, command, give และ particle
- มี preview โค้ดก่อนสร้างจริง และสร้างไฟล์ `scripts/hs_<slug>.js`
- อัปเดต script module, dependency `@minecraft/server` และ entry script ให้โดยอัตโนมัติ
- สร้าง history snapshot ก่อนเขียนไฟล์ทุกครั้ง
- API หลัก: `GET /api/scriptbuilder/events`, `POST /api/scriptbuilder/preview`, `POST /api/scriptbuilder/create`
- ข้อจำกัดปัจจุบัน: ต้องเลือกโฟลเดอร์ Behavior Pack โดยตรง การแก้ archive และการ rebuild archive จะทำในรอบถัดไป

### การเชื่อมหน้าใช้งาน

- เพิ่มเมนู sidebar สำหรับ `ประวัติไฟล์`, `ดู Asset` และ `Script Builder`
- เพิ่มทางลัดจากเมนู project context menu ไปยังทั้ง 3 ฟีเจอร์
- เพิ่ม API router ใน `app/main.py` สำหรับ `history`, `assets` และ `scriptbuilder`

### ผลการตรวจสอบรอบนี้

- Backend targeted tests: `11 passed`
- ตรวจ syntax JavaScript ของหน้าใหม่และไฟล์ projects ผ่าน
- ตรวจ Python compile ของโมดูลที่เพิ่มและไฟล์ที่เชื่อมระบบผ่าน
- ยังไม่ได้ทำ API import/live server ใน environment นี้ เพราะ Python ปัจจุบันยังไม่มี `fastapi`; ให้ใช้ environment เดิมของโปรเจกต์ที่ติดตั้ง dependencies แล้ว
- ยังไม่ได้ rebuild ไฟล์ `.exe` และทำ live desktop QA ในรอบนี้
